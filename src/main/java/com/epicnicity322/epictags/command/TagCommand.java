...
public final class TagCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length > 0 && sender.hasPermission("epictags.edit.others")) {
            toEdit = Bukkit.getPlayer(args[0]);
            if (toEdit == null) {
        ...
        if (toEdit != p) lang.send(sender, lang.get("Open Other Player Editor").replace("<player>", toEdit.getName()));
        new GroupsGUI(toEdit).open(p);
        return true;
    }
}
